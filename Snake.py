...
class ClientGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Mini Snake Game")
    ...
    def on_start(self):
        if self._singleplayer:
            ...
            if self._online:
                Thread(target=Snake, args=(True, str(self.ip_input.text()), int(self.port_input.text()))).start()
            else:
                Thread(target=Snake, args=(True,)).start()
    def on_online(self):
...
class Snake():
    ...
    def GameLoop(self):
        ...
        while self.running:
            ...
            if time.time() - t >= frametime:
                ...
                self.DrawSnake(self.snake, self.blue, self.green)
                if self.multiplayer:

                    self.sc.Send(self.snake)
                    self.enemy_snake = self.sc.Receive()
                    self.food_pos = self.enemy_snake[-2]
                    self.my_score, self.enemy_score = self.enemy_snake[-1]
                    self.Food()
                    self.enemy_snake = self.enemy_snake[:-1]
                    self.DrawSnake(self.enemy_snake, self.magenta, self.cyan)
                    score_text = font.render(f"Me: {self.my_score}  Enemy: {self.enemy_score}", True, (255, 255, 255))
    ...
    def MapBorder(self):
        ...
        pygame.draw.rect(self.screen, self.red, (border_start,
                                                   border_start, border_end, border_thickness))
        pygame.draw.rect(self.screen, self.red, (border_start,
                                                   border_start, border_thickness, border_end))
        pygame.draw.rect(self.screen, self.red, (border_end,
                                                   border_start, border_thickness, border_end))
        pygame.draw.rect(self.screen, self.red, (border_start,
